...
object ItsGpConnectorTestApp {
  ...
  def usage(): Unit = {
    ...
    println("-m | --mode <append|overwrite|..> - default append or use spark.ymatrix.mode config param")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder().appName("its-ymatrix-connector-test").getOrCreate()
    import spark.implicits._
    val sc = spark.sparkContext
    val nRowsGenerate: Int = options.getOrElse('nRows, "10000").toInt
    val ymatrixUrl = spark.conf.get("spark.ymatrix.jdbc.url", "jdbc:postgresql://al-desktop:5432/bigdata")
    val ymatrixUser = spark.conf.get("spark.ymatrix.user", "al")
    val ymatrixPassword = spark.conf.get("spark.ymatrix.password", "")
    val ymatrixTable = spark.conf.get("spark.ymatrix.dbtable", "uuid_seq")
    val ymatrixInsertMode = options.getOrElse('mode, spark.conf.get("spark.ymatrix.mode", "append"))
    val ymatrixBufferSize = spark.conf.get("spark.ymatrix.buffer.size", "")
    val ymatrixOptionsMap = MutableMap[String, String]()
    ymatrixOptionsMap.put("url", ymatrixUrl)
    ymatrixOptionsMap.put("user", ymatrixUser)
    ymatrixOptionsMap.put("dbtable", ymatrixTable)
    if (ymatrixPassword.nonEmpty)
      ymatrixOptionsMap.put("password", ymatrixPassword)
    if (ymatrixBufferSize.nonEmpty)
      ymatrixOptionsMap.put("buffer.size", ymatrixBufferSize)
    ...
    df.write.format("its-ymatrix")
      //.option("url", ymatrixUrl).option("user", "al").option("dbtable", "uuid_seq")
      .options(ymatrixOptionsMap.toMap)
      .mode(ymatrixInsertMode) //SaveMode.Append)
      .save()
    println(s"Done")
  }
}
